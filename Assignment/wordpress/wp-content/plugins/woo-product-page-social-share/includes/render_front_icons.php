...
if ($options['wpss_show_hide_field'] == 'on') {
	...
	function wpss_social_addtoany_js() {
		...
		switch ($options['wpss_buttons_position_field']) {
...
function wpss_render_social_buttons_front(){
	...
	global $wpss_position,$options,$wpss_backgrounds;
	...
	if ($wpss_position == 55 || $wpss_position == 8 || $wpss_position == 3 || $wpss_position == 25 || $wpss_position == 35) {
		...
		$social_services = explode(",",$options['wpss_buttons_list_field']);

		foreach ($social_services as $value) {
			if (!empty($value)) {

				if (isset($options['wpss_buttons_icontext_field']) && $options['wpss_buttons_icontext_field'] == 'text_icons') {
				...
				if (isset($options['wpss_buttons_style_field']) && $options['wpss_buttons_style_field'] == 'square' && $options['wpss_buttons_icontext_field'] == 'icons_only') {
					echo '<style type="text/css">
								.a2a_svg, .a2a_count { border-radius: 0 !important; }
						  </style>';
				}elseif(isset($options['wpss_buttons_style_field']) && $options['wpss_buttons_style_field'] == 'circle'  && $options['wpss_buttons_icontext_field'] == 'icons_only'){
					echo '<style type="text/css">
